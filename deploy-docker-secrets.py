...
SOURCE = os.path.join(HOME, 'projects', 'wps12', 'selfstudy', 'django', 'deploy-test')
...
def server_init():
    ...
    print('1111-------222222222222')


# 2.실행중인 컨테이너 종료, pull, run
def server_pull_run():
    ssh_run(f'sudo docker stop deploy-test', ignore_error=True)
...
def copy_secrets():
    ...
    ssh_run(f'sudo docker cp /tmp/secrets.json deploy-test:/srv/deploy-test')
    print('333333333333333333')


# 4. Container에서 collectstatic, supervisor실행
def server_cmd():
    ssh_run(f'sudo docker exec deploy-test /usr/sbin/nginx -s stop', ignore_error=True)
    ssh_run(f'sudo docker exec deploy-test python3 manage.py collectstatic --noinput')
    ssh_run(f'sudo docker exec -it -d deploy-test '
            f'supervisord -c /srv/deploy-test/.config/supervisord.conf -n')
